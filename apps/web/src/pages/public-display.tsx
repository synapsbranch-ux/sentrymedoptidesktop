  ...
  return <main className="min-h-dvh bg-zinc-950 p-4 text-white sm:p-6 lg:p-8">
    ...
    <div className="mt-6 grid min-w-0 gap-6 xl:grid-cols-[1.15fr_.85fr]">
      <section className="min-w-0 rounded-2xl border border-white/15 bg-white/[.04] p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-emerald-400">{i18n.t("Now serving")}</p><h2 className="mt-1 text-2xl font-bold">{i18n.t("Current progress")}</h2></div><Users className="h-7 w-7 text-zinc-500" /></div><div className="mt-5 grid gap-3">{nowServing.length ? nowServing.map((item) => <div className="grid gap-3 rounded-xl bg-white p-4 text-zinc-950 sm:grid-cols-[1fr_auto] sm:items-center" key={item.code}><div><div className="font-mono text-2xl font-bold sm:text-3xl">{item.patientLabel}</div><div className="mt-1 text-sm text-zinc-500">{item.doctor ? `${i18n.t("Assigned to")} ${item.doctor}` : i18n.t("Clinic team")}</div></div><div className="rounded-full bg-zinc-950 px-4 py-2 text-center text-sm font-bold text-white">{i18n.t(stageLabels[item.stage] ?? item.stage)}</div></div>) : <div className="grid min-h-40 place-items-center rounded-xl border border-dashed border-white/20 text-center text-zinc-400">{i18n.t("No patient is being served right now.")}</div>}</div>
        <div className="mt-7 flex items-center justify-between"><h3 className="font-bold">{i18n.t("Waiting queue")}</h3><span className="rounded-full bg-white/10 px-3 py-1 font-mono text-sm">{waiting.length}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{waiting.map((item, index) => <div className="flex items-center gap-4 rounded-xl border border-white/10 bg-black/20 p-4" key={item.code}><div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/10 font-mono font-bold">{index + 1}</div><div className="min-w-0 flex-1"><div className="truncate font-mono text-lg font-bold">{item.patientLabel}</div><div className="mt-1 flex items-center gap-1 text-xs text-zinc-400"><Clock3 className="h-3 w-3" />{i18n.t("Arrived")} {formatTime(item.arrivedAt)}</div></div><span className="text-xs font-semibold text-zinc-300">{i18n.t(stageLabels[item.stage] ?? item.stage)}</span></div>)}</div>
      </section>
