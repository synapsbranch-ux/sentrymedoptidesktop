...
export function SignatureSettings() {
  ...
  const save = async (blob: Blob, method: "uploaded" | "drawn", filename: string) => {
    setBusy(true);
    try {
      const form = new FormData();
      form.append("method", method);
      form.append("signature", blob, filename);
      ...
      <CardContent className="grid gap-4">
        {state.loading ? <Skeleton className="h-32" /> : state.error ? <ErrorState message={state.error.message} retry={state.reload} /> : (
          <>
            ...
            <SignatureUpload busy={busy} onFile={(file) => save(file, "uploaded", file.name)} />
            <SignaturePad busy={busy} onDrawn={(blob) => save(blob, "drawn", "signature.png")} />
          </>
        )}
      </CardContent>
    </Card>
