...
beforeEach(() => {
  ...
  if (!Blob.prototype.arrayBuffer) Object.defineProperty(Blob.prototype, "arrayBuffer", { value() { return Promise.resolve(new TextEncoder().encode("png").buffer); } });
...
describe("SignatureSettings", () => {
  it("saves only to the signed-in user's own signature without using a multipart body", async () => {
    ...
    fireEvent.pointerUp(canvas, { pointerId: 1 });
    fireEvent.click(screen.getByRole("button", { name: "Save drawn signature" }));
    await waitFor(() => expect(put).toHaveBeenCalledWith("/me/signature", expect.objectContaining({ method: "drawn", data: expect.any(String) })));
    ...
    const body = put.mock.calls[0][1] as { method: string; data: string };
