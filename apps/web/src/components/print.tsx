...
export function PrintHeader({ documentTitle, number, date }: { documentTitle: string; number: string; date: string }) {
  ...
  return <header className="flex items-start justify-between gap-6 border-b-2 border-black pb-5">
    <div className="flex min-w-0 items-start gap-4">
      ...
      <div><h1 className="text-xl font-bold">{clinic.name || "Clinic"}</h1><p className="mt-1 max-w-md whitespace-pre-line text-xs text-zinc-600">{[clinic.address, clinic.phone, clinic.email].filter(Boolean).join(" · ")}</p>{clinic.nif && <p className="mt-1 font-mono text-xs">NIF: {clinic.nif}</p>}</div>
    </div>
