...
func signatureJSONRequest(t *testing.T, method string, content []byte) *http.Request {
	t.Helper()
	body, err := json.Marshal(map[string]string{"method": method, "data": base64.StdEncoding.EncodeToString(content)})
	if err != nil {
		t.Fatal(err)
	}
	request := httptest.NewRequest(http.MethodPut, "/api/v1/me/signature", bytes.NewReader(body))
	request.Header.Set("Content-Type", "application/json")
	request.RemoteAddr = "127.0.0.1:1234"
	return request
}

func TestSignatureCanBeSavedThroughDesktopSafeJSON(t *testing.T) {
	a := newTestApp(t)
	saved := a.requestRaw(signatureJSONRequest(t, "drawn", signaturePNG), a.doctor)
	if saved.Code != http.StatusOK {
		t.Fatalf("JSON signature save: %d %s", saved.Code, saved.Body.String())
	}
	image := a.request(http.MethodGet, "/api/v1/me/signature/image", nil, a.doctor)
	if image.Code != http.StatusOK || !bytes.Equal(image.Body.Bytes(), signaturePNG) {
		t.Fatalf("stored JSON signature: %d", image.Code)
	}
}
