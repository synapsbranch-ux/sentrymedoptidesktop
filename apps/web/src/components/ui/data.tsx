...
export function Badge({ children, tone = "neutral", className }: { children: React.ReactNode; tone?: "neutral" | "success" | "warning" | "danger"; className?: string }) {
  ...
  return <span className={cn("inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold capitalize", tones[tone], className)}>{children}</span>;
}
...
export function ErrorState({ message, retry }: { message: string; retry?: () => void }) { return <div role="alert" className="min-w-0 break-words rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-900"><strong>This section could not connect to the clinic server.</strong><p className="mt-1">{message}</p><p className="mt-2 text-xs opacity-80">On a phone or tablet, verify the clinic Wi-Fi and keep the SentryMed desktop server running.</p>{retry && <button className="mt-2 min-h-11 font-semibold underline" onClick={retry}>Reconnect and try again</button>}</div>; }
