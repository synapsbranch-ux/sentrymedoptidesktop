...
func (s *Server) handleSignatureSave(w http.ResponseWriter, r *http.Request) {
	...
	hash := sha256.New()
	size, copyErr := io.Copy(io.MultiWriter(output, hash), io.LimitReader(io.MultiReader(bytes.NewReader(prefix), file), maxSignatureBytes+1))
	...
	_, err = s.db.ExecContext(r.Context(), `INSERT INTO user_signatures(user_id,method,storage_name,media_type,size_bytes,checksum_sha256,created_at,updated_at)
		...
		user.ID, method, storageName, detected, size, hex.EncodeToString(hash.Sum(nil)), now, now)
	if err != nil {
		_ = os.Remove(path)
