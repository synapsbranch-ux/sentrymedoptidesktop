...
func (s *Server) handleLabOrdersList(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		var version int
		if rows.Scan(&id, &number, &patientID, &mrn, &patientName, &prescriptionID, &invoiceID, &supplierID, &supplierName, &frameID, &frameName, &lensID, &lensName, &lensType, &material, &coatings, &tint, &treatments, &measurements, &notes, &orderedAt, &expectedAt, &cost, &price, &status, &deliveredAt, &version, &createdAt, &updatedAt) == nil {
